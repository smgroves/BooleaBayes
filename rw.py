...
def random_walks(attractor_dict, rules, regulators_dict, nodes, save_dir, radius=2, perturbations=False, 
                 iters=1000, max_steps=500, stability=False, reach_or_leave="leave", random_start=False, 
                 on_nodes=None, off_nodes=None, basin=None):
    """
    ...
    if type(radius) == list:
        ...
        raise Eception("Radius must be an integer greater than 0 or a list of integers.")
    ...
    for k in attractor_dict.keys():
        ...
        for radius_ in radius:
            for start_idx in steady_states:
                ...
                outfile = open(op.join(save_dir, f"walks/%d/results_radius_{radius_}.csv" % start_idx), "w+")
                out_len = open(op.join(save_dir, f"walks/%d/len_walks_{radius_}.csv" % start_idx), "w+")
                ...
                for iter_ in range(iters):
                    ...
                    if reach_or_leave == "leave":
                        walk, counts, switches, distances = random_walk_until_leave_basin(start_idx, rules,
                                                                                    regulators_dict, nodes,
                                                                                    radius_, max_steps=max_steps,
                                                                                    on_nodes=on_nodes, off_nodes=off_nodes)
                    elif reach_or_leave == "reach":
                        walk, counts, switches, distances = random_walk_until_reach_basin(start_idx, rules, regulators_dict, 
                                                                      nodes, radius=radius_, max_steps=max_steps, 
                                                                      on_nodes=on_nodes,off_nodes=off_nodes, basin=basin)
                    else: 
                        raise Exception("Value for `reach_or_leave` must be string 'reach' or 'leave'.")
            ...
            if perturbations:
                ...
                if on_nodes == None and off_nodes == None:
                    ...
                    outfile = open(op.join(save_dir, f"perturbations/%d/results.csv" % start_idx),"w+")
                    ...
                    for expt_node in nodes:
                        ...
                        for iter_ in range(iters):
                            ...
                            if reach_or_leave == "leave":
                                walk_on, counts_on, switches_on, distances_on = random_walk_until_leave_basin(start_idx,
                                                                                        rules,regulators_dict,nodes,radius,
                                                                                        max_steps=500,on_nodes=[expt_node, ],
                                                                                        off_nodes=[])
                            # elif reach_or_leave == "reach":
                        ...
                        loc_0, loc_1, stabilized = plot_histograms(n_steps_to_leave_0, n_steps_activate, 
                                                       expt, bins=60,
                                                       fname=op.join(save_dir,"perturbations/%d/%s.pdf" % (start_idx, expt)))
                        
                        outfile.write(op.join(save_dir, "perturbations/%d,%s,%s,activate,%f\n" % (start_idx, phenotype_name,
                                                                                              expt_node, stabilized)))
                        expt = "%s_knockdown" % expt_node
                        for iter_ in range(iters):
                            walk_off, counts_off, switches_off, distances_off = random_walk_until_leave_basin(
                        ...
                        loc_0, loc_1, stabilized = plot_histograms(n_steps_to_leave_0, n_steps_knockout, expt, bins=60,
                                                       fname=op.join(save_dir,"perturbations/%d/%s.pdf" % (start_idx,expt)))
                        outfile.write(op.join(save_dir, "perturbations/%d,%s,%s,knockdown,%f\n" % 
...
def random_walk_until_leave_basin(start_state, rules, regulators_dict, nodes, 
                                  radius=2, max_steps=10000, on_nodes=[], off_nodes=[]):
    """
    Parameters
    ----------
    start_state : int
    ...
    while (distance <= radius and step_i < max_steps):
        ...
        for node_i, node in enumerate(nodes):
            if node in on_nodes+off_nodes: 
                continue
            neighbor_idx, flip = ut.update_node(rules, regulators_dict, node, node_i, nodes, node_indices, next_step)
            r = r - flip**2/(1.*nu)
            if r <= 0:
                next_step = [{'0':False,'1':True}[i] for i in ut.idx2binary(neighbor_idx, n)]
...
def random_walk_until_reach_basin(start_state, rules, regulators_dict, nodes, 
                radius=2, max_steps=10000, on_nodes=[],off_nodes=[], basin=1):
    """
    Parameters
    ----------
    start_state :  
