...
def get_rules_scvelo(data, data_t1, vertex_dict, plot=True, show_plot=True, save_plot=True, threshold=0.1, 
                     save_dir="rules", hlines=None):
    v_names = dict()
    # Invert the vertex_dict
    for vertex_name in list(vertex_dict): v_names[vertex_dict[vertex_name]] = vertex_name 
    ...
    for gene in nodes:
        ...
        if len(regulators) > 0:
            ...
            if plot: 
                plot_rule(gene, rules[gene], regulators, heat, data, save_dir=save_dir, 
                          save=save_plot, show_plot=True, hlines=hlines)
...
def roc(validation, node, n_thresholds=10, fname='ROC_plots', plot=False, 
        show_plot=False, save=False, save_dir=None):
    tprs = []
    fprs = []
    ...
    if plot == True:
        plot_roc(fprs, tprs, area, node, save=save, save_dir=save_dir, 
                 fname=fname, show_plot=show_plot)
...
def fit_validation(data_test, data_test_t1, nodes, regulators_dict, rules, val_type='scvelo', save=False, 
                   save_dir=None, fname='', phenotypes=None, clusters=None, plot=False,
                   plot_clusters=False, show_plots=False, save_df=False, n_thresholds=50, 
    ...
    for node in nodes:
        ...
        if val_type == 'scvelo':
            validation = plot_accuracy_scvelo(data_test, data_test_t1, node, regulators_dict, rules, 
                                        phenotypes=phenotypes, clusters=clusters, plot_clusters=plot_clusters, 
                                        save=save, save_dir=save_dir, fname=fname, show_plot=show_plots, 
                                        save_df=save_df, customPalette=customPalette)
        # call the other accuracy function -- Not tested yet **
        else:
            validation = plot_accuracy(data, node, regulators_dict, rules, phenotypes=phenotypes, 
                                    plot_clusters=plot_clusters, save_dir=save_dir, clusters=clusters, 
                                    fname=fname, plot=show_plots, save=save, save_df=save_df,
                                    customPalette=customPalette)
                
        tprs, fprs, area = roc(validation, node, n_thresholds=n_thresholds, fname=fname, 
                               plot=plot, show_plot=show_plots, save=save, save_dir=save_dir)
        tpr_all[node] = tprs
        fpr_all[node] = fprs
...
def roc_from_file(validation_dir, nodes, n_thresholds=50, plot=False, show_plots=False, 
                    save=False, save_dir=None, fname=''):
    ...
    for node in nodes: 
        validation = pd.read_csv(f'{validation_dir}/{node}_validation.csv',
...
def find_attractors(binarized_data, rules, nodes, regulators_dict, tf_basin, 
                    save_dir=None, threshold = 0.5, on_nodes = [], off_nodes = []):
    att = dict()
    n = len(nodes)
    dist_dict = None
    ...
    for k in binarized_data.keys():
        # print(k)
        att[k] = []
        outfile = open(f'{save_dir}/attractors_{k}.txt', 'w+') #if there are no  clusters, comment this out
        ...
        for i in start_states:
            ...
            if type(tf_basin) == int and tf_basin >= 0:
                if len(on_nodes) == 0 and len(off_nodes) == 0:
                    stg, edge_weights = ut.get_partial_stg(start_states, rules, nodes, regulators_dict, tf_basin)
                else:
                    stg, edge_weights = ut.get_partial_stg(start_states, rules, nodes, regulators_dict, tf_basin,
            ...
            # elif type(tf_basin) == dict:
                ...
                if len(on_nodes) == 0 and len(off_nodes) == 0:
                    stg, edge_weights = ut.get_partial_stg(start_states, rules, nodes, regulators_dict, dist_dict[k])
                else:
                    stg, edge_weights = ut.get_partial_stg(start_states, rules, nodes, regulators_dict, dist_dict[k],
                                                           on_nodes=on_nodes, off_nodes=off_nodes)
            else: 
                print("tf_basin needs to be an integer or < 0 to indicate dictionary of integers for each subtype.")
            ...
            d_stg = ut.prune_stg_edges(stg, edge_weights, n - len(on_nodes)-len(off_nodes), threshold=threshold)
            ...
            for v in stg.vertices():
                ...
                if components[2][components[0][v]]:
                    if v != 0:
                        outfile.write(f"{start_states[0]},{ut.hamming_idx(vidx[v],start_states[0],n)}, {vidx[v]}\n")
...
def filter_attractors(phenotypes, #average_states, attractor_dict, 
                      avg_state_idx_dir, attractor_dir, save_dir, nodes):
    # TEST
    average_states={'Tumor1': [], 'Tumor2': []}
    attractor_dict={'Tumor1': [], 'Tumor2': []}
    for phen in phenotypes:
        d = pd.read_csv(f'{avg_state_idx_dir}/average_states_idx_{phen}.txt', sep=',', header=0)
